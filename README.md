...
**Status:** v0.4.1 — budget-aware subagent trees with pause/handoff files, autonomous 5h-block renewal (auto-save + scheduled auto-wake), context auto-save, plus everything from 0.3: named checkpoint lanes, time & AFK awareness, cross-session budget awareness, worktree-aware checkpoints, and AFK cache keepalive. See the [changelog](CHANGELOG.md).
